...
`B 新开50+授权` 先保存50个持久化名额，后台绑定目标 workspace 的主区域 pane。重复按 B 复用未完成批次；完成后再次按 B 创建新批次。全系统共享4个初始化许可，相邻创建至少间隔0.5秒；标签页已创建、原生 Codex 尚未就绪时仍占用许可。启动租约30秒后让出许可并继续核对原名额，不能重发回执不明的创建请求。
